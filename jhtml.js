...
(function ($, window) {
    ...
    jHtmlArea.fn = jHtmlArea.prototype = {
        ...
        updateTextArea: function () {
	        // This change only updates textarea contents when
	        // placeholder is not in place - ChrisM
	        if (!$(this.editor.body).find('span#placeholder').length) {
		        this.textarea.val(this.toHtmlString());
		    	// This is a change to the source code, used
        		// so that our own method of retaining cell changes 
        		// is invoked - NATHAN
				$(this.textarea).trigger("keyup.editablecellchange");
	        }
        },
    ...
    return html.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '')
            ...
            edit.designMode = 'on';
                edit.open();
               var sanitizedContent = htmlSanitize(this.textarea.val());
    edit.write('<body>' + sanitizedContent + '</body>');
                edit.close();
            if (options.css) {
        ...
        initToolBar: function (options) {
            ...
            var menuItem = function (className, altText, action) {
                return $("<li/>").append($("<a/>").addClass(className).attr("title", altText).click(function () { action.call(that, $(this)); }));
            };
        ...
        attachEditorEvents: function () {
            ...
            this.textarea.click(fnHA).
                ...
                blur(function () {
                    fnTA();
                    // Another change to the source code to behave
                    // like other related column properties
